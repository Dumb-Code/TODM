...
public class AnimationPass<T extends EntityAnimatable>
{
    ...
    private void initAnimation(Animation animation)
    {
        ...
        {
            this.animation = AnimalAnimations.IDLE.get();
        }
    }
    ...
    private float calculateInertiaFactor()
    {
        ...
        if (this.useInertia && AnimalAnimations.getAnimation(this.animation).useInertia())
        {
    ...
    private boolean updateAnimationTick(T entity, float ticks)
    {
        ...
        if (!AnimalAnimations.getAnimation(this.animation).shouldHold() || this.poseIndex < this.poseCount)
        {
    ...
    private void initAnimationTicks(T entity)
    {
        ...
        if (AnimalAnimations.getAnimation(this.animation).shouldHold())
        {
            this.poseIndex = this.poseCount - 1;
    ...
    private void onPoseFinish(T entity, float ticks)
    {
        ...
        {
            this.setAnimation(entity, this.isEntityAnimationDependent() ? AnimalAnimations.IDLE.get() : this.getRequestedAnimation(entity));
        } else
        {
    ...
    private boolean incrementPose()
    {
        ...
        if (this.poseIndex >= this.poseCount)
        {
            AnimalAnimations animation = AnimalAnimations.getAnimation(this.animation);
    ...
    private void setAnimation(T entity, Animation requestedAnimation)
    {
        ...
        if (this.animations.get(requestedAnimation) != null && !(this.animation != AnimalAnimations.IDLE.get() && this.animation == requestedAnimation && !this.isLooping()))
        {
